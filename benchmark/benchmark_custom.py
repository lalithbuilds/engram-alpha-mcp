...
def benchmark_4way_rrf_vs_fts():
    ...
    if successes > 0:
        print(f"✅ {successes} 4-Way RRF Hybrid Searches: {elapsed_rrf:.4f}s ({successes/elapsed_rrf:.1f} queries/sec, {elapsed_rrf/successes*1000:.2f}ms/query)")
    else:
        print("❌ All 4-Way RRF Hybrid Searches failed.")
...
def benchmark_recursive_cte_graph():
    ...
    if successes > 0:
        print(f"✅ {successes} Recursive CTE 4-Hop Graph Traversals: {elapsed:.4f}s ({successes/elapsed:.1f} traversals/sec)")
    else:
        print("❌ All Recursive CTE Graph Traversals failed.")
