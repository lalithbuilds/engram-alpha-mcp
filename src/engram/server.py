...
def escape_fts(text: str) -> str:
    """Strip special FTS5 chars to prevent syntax errors."""
...
def search_memory(query: str, limit: int = 5) -> str:
    ...
    # ACT-R Power-Law time weight: POWER(1.0 + (0.1 * MAX(0, days_old)), -0.5)
    sql = """
    SELECT n.id, n.content, rank,
           (rank * POWER(1.0 + (0.1 * MAX(0, (julianday('now') - julianday(COALESCE(NULLIF(n.last_accessed_at, ''), n.created_at))))), -0.5)) as final_score
